...
- ⚠️ **Generación de XML UBL 2.1** (`sunat-fe-xml`, Factura/Boleta only): implemented, unit-tested, structurally well-formed, and now confirmed to reach SUNAT's real BETA parser/validator (see signature note below). SUNAT rejects it on content grounds — `SOAP Fault 3244: Debe consignar la informacion del tipo de transaccion del comprobante` — meaning at least one required business element (e.g. `cac:PaymentTerms` / forma de pago) is missing. Not yet fixed.
- ✅ **Firma XML-DSig** (`sunat-fe-signer`): implemented using `xml-crypto` (RSA-SHA1 + classic C14N, enveloped-signature transform, `URI=""`). Previously rejected by SUNAT BETA with `SOAP Fault 2335: Incorrect reference digest value`; the real causes were structural, not algorithmic — the reference XPath (`//*[...]`) was also matching the nested `cbc:Note` element (generating two `<ds:Reference>` instead of one), and the default `Id`-based reference added an `Id` attribute SUNAT's XSD doesn't declare. Both are fixed (see `firmar.ts` for the full account and `firmar.test.ts` for the regression tests) and **verified against the real SUNAT BETA server**: the signature/schema error is gone, replaced by the unrelated content error above — proof SUNAT now parses and cryptographically verifies the document.
- ✅ **Cliente SOAP** (`sunat-fe-client`): implemented and **verified end-to-end against the real SUNAT BETA server** — zip packaging (with the correct ISO-8859-1 byte encoding SUNAT's UBL-PE profile requires, a real encoding bug caught and fixed during this work), WS-Security UsernameToken auth, the `sendBill` SOAP call, and SOAP-Fault/CDR parsing all correctly reach SUNAT and correctly interpret its real response.
- 📦 **CLI / npm package (`sunat-fe`)**: structured to be publishable (bin, exports, package.json) but **not published** to the npm registry. Commands: `catalogo`, `calcular`, `generar-xml`, `firmar`, `enviar`, `estado` — the pipeline now reaches SUNAT and gets past signature verification, but still will not yet produce an ACEPTADA CDR, per the content gap above.
